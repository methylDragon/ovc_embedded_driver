...
VDMADriver::VDMADriver(int uio_num, int cam_num, const std::vector<uint8_t>& sample_msg, size_t img_size)
{
  ...
  frame_offset = sample_msg.size() - img_size + 22;
  std::cout << "msg size = " << sample_msg.size() << "img = " << img_size << std::endl;
  std::cout << "Frame offset = " << frame_offset << std::endl;
  configureVDMA();
  for (int i=0; i<NUM_FRAMEBUFFERS; ++i)
  {
    ...
    std::cout << "Opening file " << memory_filename << std::endl;
    ...
    memcpy((void *)memory_mmap[i], dummy, sizeof(dummy));
    // Prefill message
...
void VDMADriver::setFramebuffer(int i, const std::string& memory_filename)
{
  ...
  while (!map_file.eof())
  {
    ...
    {
      std::cout << "Line for " << memory_filename << " = " << line << std::endl;
      std::string address_str(line.substr(0,8));
      int memory_address = std::stoi(address_str, 0, 16);
      std::cout << "Got memory address = " << memory_address << std::endl;
      ...
      std::cout << "Physical address is " << phys_addr << std::endl;
...
void VDMADriver::sendFramebuffer(int fb_num, uint32_t address)
{
  ...
  //writeRegister(START_ADDR_0 + fb_num, address);
...
void VDMADriver::setHeader(const std::vector<uint8_t>& header, int index)
{
  if (index == -1)
    index = last_fb;
  memcpy((void *)memory_mmap[index], &header[0], header.size());
}
